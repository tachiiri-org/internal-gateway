...
__name(isGatewayError, "isGatewayError");
function toGatewayError(error, requestId) {
...
__name(toGatewayError, "toGatewayError");
...
// src/policies/auth/actor.ts
var ACTOR_SCOPE_SEPARATOR = " ";
function actorFromJwt(payload) {
  if (!payload.sub) {
  ...
  const rawIssuer = payload.iss ? { iss: payload.iss, aud: payload.aud } : void 0;
  return {
    kind: "user",
    sub: payload.sub,
    scopes,
...
__name(actorFromJwt, "actorFromJwt");
function actorToHeaders(actor) {
...
__name(actorToHeaders, "actorToHeaders");
...
// src/policies/auth/verifyJwt.ts
...
// src/policies/internal/requireInternalToken.ts
...
__name(assertInternalToken, "assertInternalToken");

// src/policies/rateLimit/key.ts
function rateLimitKey(actor, request) {
...
// src/policies/rateLimit/limiter.ts
var WINDOW_MS = 6e4;
var RATE_LIMITS = {
  read: 120,
  write: 60,
...
function enforceRateLimit(key, rateClass) {
  const now = Date.now();
...
// src/policies/internal/sanitizeActorHeaders.ts
var ACTOR_HEADERS = [
  "x-actor-kind",
  "x-actor-sub",
  "x-actor-scopes",
...
function sanitizeActorHeaders(headers) {
  for (const header of ACTOR_HEADERS) {
    headers.delete(header);
  }
}
__name(sanitizeActorHeaders, "sanitizeActorHeaders");

// src/upstreams/r2.ts
async function proxyToR2(params) {
...
__name(readUpstreamBody, "readUpstreamBody");

// src/upstreams/github.ts
async function proxyToGithub(params) {
  const { request, env, upstreamPath, actor, requestId, routeDef } = params;
  const targetUrl = new URL(upstreamPath, "https://github-service.internal");
  const headers = new Headers(request.headers);
  sanitizeActorHeaders(headers);
  headers.delete("x-internal-token");
  if (actor) {
  ...
  let response;
  try {
    response = await env.GITHUB_SERVICE.fetch(upstreamRequest);
  } catch (error) {
    throw new GatewayError({
      status: 502,
      code: "upstream_unreachable",
  ...
  if (!response.ok) {
    const upstreamBody = await readUpstreamBody2(response);
    throw new GatewayError({
      status: response.status,
      code: "upstream_error",
      message: "Upstream request failed",
...
__name(proxyToGithub, "proxyToGithub");
async function readUpstreamBody2(response) {
  const contentType = response.headers.get("content-type") ?? "";
  try {
    if (contentType.includes("application/json")) {
      return await response.json();
    ...
    return { error: "failed_to_read_.body", cause: error };
  }
}
__name(readUpstreamBody2, "readUpstreamBody");

// src/upstreams/googleDrive.ts
async function proxyToGoogleDrive(params) {
  const { request, env, upstreamPath, actor, requestId, routeDef } = params;
  const targetUrl = new URL(upstreamPath, "https://google-drive-service.internal");
  const headers = new Headers(request.headers);
  sanitizeActorHeaders(headers);
  headers.delete("x-internal-token");
  if (actor) {
  ...
  let response;
  try {
    response = await env.GOOGLE_DRIVE_SERVICE.fetch(upstreamRequest);
  } catch (error) {
    throw new GatewayError({
      status: 502,
      code: "upstream_unreachable",
  ...
  if (!response.ok) {
    const upstreamBody = await readUpstreamBody3(response);
...
__name(proxyToGoogleDrive, "proxyToGoogleDrive");
async function readUpstreamBody3(response) {
  const contentType = response.headers.get("content-type") ?? "";
  try {
    if (contentType.includes("application/json")) {
      return await response.json();
...
__name(readUpstreamBody3, "readUpstreamBody");
...
async function handleV1Request(params) {
...
function matchV1Route(method, pathname) {
...
__name(matchV1Route, "matchV1Route");
...
// .wrangler/tmp/bundle-OtggQC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-OtggQC/middleware-loader.entry.ts
